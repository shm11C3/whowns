...
`which` や `command -v` が答えるのは「`PATH` からどの実行ファイルが見つかるか」です。`which -a` なら候補を列挙できますが、それぞれをどのパッケージ・バージョン管理ツールが所有し、どう保守すべきかは分かりません。`whowns` は `PATH` 探索の先を担い、有効な実体と隠れた実体を、所有チェーン、判断根拠、次に使う管理コマンドへ結びつけます。
