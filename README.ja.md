...
`which` は、コマンドが `PATH` 上のどの実行ファイルに解決されるかを調べるためによく使われます。`which -a` で一致する実行ファイルをすべて列挙しても、それぞれをどのパッケージ・バージョン管理ツールが所有し、どう保守すべきかは分かりません。`whowns` は `PATH` 探索の先を担い、有効な実体と隠れた実体を、所有チェーン、判断根拠、次に使う管理コマンドへ結びつけます。
