...
fn upstream_owner(
    ...
    if manager == OwnerId::Nvm {
        return Some(source_from_root(manager, nvm_root(runtime_path), runner));
    }
    if manager == OwnerId::Sdkman {
        return Some(source_from_root(manager, sdkman_root(runtime_path), runner));
    }
...
fn source_from_root(
    ...
    let source = detect::detect(manager.display_name(), &root, &real_root, runner);
    if source.id == manager {
